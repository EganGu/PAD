...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        ref_model = AutoModelForCausalLM.from_pretrained(ref_model, **model_init_kwargs).eval()
    ...
    def pd_loss(
        ...
        student_logps_ref: torch.FloatTensor,
        ...
        rewards = student_logps - student_logps_ref
        logits = (rewards[range(batch_size), pref_idx] - rewards[range(batch_size), dispref_idx]).to(self.accelerator.device)
    ...
    def get_batch_loss_metrics(
        ...
        with torch.no_grad():
            forward_output_ref = self.concatenated_forward(self.ref_model, batch)
            _, logps_ref = forward_output_ref[:2]
        scores = batch['scores_avglogp'] * self.alpha + batch['scores_mcq'] * (1 - self.alpha) if self.loss_type == "pd" else batch['scores_mcq']
        loss_output = self.pd_loss(logps, logps_ref, scores)
